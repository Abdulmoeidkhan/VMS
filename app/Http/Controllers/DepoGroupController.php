...
class DepoGroupController extends Controller
{
    ...
    public function depoGuestRender($id)
    {
        ...
        // return [$depo];
        return view('pages.depoGroup', ['GuestCount' => $GuestData, 'id' => $id, 'depo' => $depo, 'depoGuestRemaing' => $depoGuestRemaing]);
    }
    ...
    public function getDepoGuest($id)
    {
        ...
        foreach ($depoGroupsGuest as $key => $guest) {
            $depoGroupsGuest[$key]->depoName = DepoGroup::where('uid', $guest->depo_uid)->first(['depo_rep_name', 'depo_category']);
            $depoGroupsGuest[$key]->pictureUrl = asset('storage/images/'. $guest->uid . '.png');
            // $depoGroupsGuest[$key]->pictureUrl = 'https://res.cloudinary.com/dj6mfrbth/image/upload/v'.time().'/Images/' . $guest->uid . '.png';
